 ...
 * @since 1.0.0
 */
class techalgospotlight_Plugin_Utilities
{
	...
	/**
	 * Main techalgospotlight Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return techalgospotlight_Plugin_Utilities
	 */
	public static function instance()
	{

		if (!isset(self::$instance) && !(self::$instance instanceof techalgospotlight_Plugin_Utilities)) {
	...
	public function activate_plugin($plugin)
	{
		...
		if (!current_user_can('activate_plugins')) {
			return new WP_Error(
				...
				esc_html__('Current user can\'t activate plugins', 'techalgospotlight')
			);
		}
		...
		if (empty($plugin)) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__('Missing plugin data.', 'techalgospotlight')
			);
		}
		...
		if (empty($plugin_data)) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__('Plugin %s is not installed.', 'techalgospotlight'),
					$plugin
				)
			);
		}
		...
		do_action('techalgospotlight_plugin_activated_' . $plugin);
	}
	...
	public function deactivate_plugin($plugin)
	{
		...
		if (empty($plugin_data)) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__('Plugin %s is not active.', 'techalgospotlight'),
					$plugin
				)
			);
		}
		...
		do_action('techalgospotlight_plugin_deactivated_' . $plugin);
	}
	...
	public function get_recommended_plugins()
	{
		...
		$plugins = array(
			'hester-core' => array(
				'name' => 'Hester Core',
				'slug' => 'hester-core',
				'desc' => 'The Hester Core plugin adds extra functionality to techalgospotlight theme, such as Demo Library, widgets, custom blocks and more.',
				'thumb' => 'https://ps.w.org/hester-core/assets/icon-256x256.png'
			)
		);

		return apply_filters('techalgospotlight_recommended_plugins', $plugins);
	}
 ...
 *
 * Example: <?php $techalgospotlight_plugin_utilities = techalgospotlight_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function techalgospotlight_plugin_utilities()
{
	return techalgospotlight_Plugin_Utilities::instance();
}
