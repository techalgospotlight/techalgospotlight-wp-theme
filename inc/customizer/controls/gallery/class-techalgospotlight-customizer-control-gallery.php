 ...
 */
class techalgospotlight_Customizer_Control_Gallery extends techalgospotlight_Customizer_Control
{
	 ...
	 * @var string
	 */
	public $type = 'techalgospotlight-gallery';
	...
	public function __construct($manager, $id, $args = array())
	{
		...
		$this->button_labels = wp_parse_args(
			$this->button_labels,
			array(
				'select' => __('Select Images', 'techalgospotlight'),
				'change' => __('Modify Gallery', 'techalgospotlight'),
				'default' => __('Default', 'techalgospotlight'),
				'remove' => __('Remove', 'techalgospotlight'),
				'placeholder' => __('No images selected', 'techalgospotlight'),
				'frame_title' => __('Select Gallery Images', 'techalgospotlight'),
				'frame_button' => __('Choose Images', 'techalgospotlight'),
			)
		);
	}
		...
		?>
				<# _.defaults( data, <?php echo wp_json_encode($data); ?> ); data.input_id='input-' + String( Math.random() ); #>
					...
					<# if ( data.attachments ) { #>
						...
						<# } #>
							<div>
								<button type="button" class="button upload-button" id="image-gallery-modify-gallery">{{
									data.button_labels.change }}</button>
							</div>
