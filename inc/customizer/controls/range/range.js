(function ($) {
  ...
  wp.customize.controlConstructor["techalgospotlight-range"] =
    wp.customize.Control.extend({
      ...
      save: function () {
        ...
        if (this.container.find(".techalgospotlight-control-unit").length) {
          value.unit = this.container
  ...
  (function ($, undef) {
    ...
    RangeControl = {
      ...
      _addListeners: function () {
        ...
        self.element.on("click", "input[type=range]", function () {
          range = $(this);
          input = range.parent().children(".techalgospotlight-range-input");
         ...
         * @since 1.0.0
         */
        self.element.on("click", ".techalgospotlight-reset-range", function () {
          var $el;
      ...
      _initResponsiveControls: function () {
        var self = this,
          el = self.element,
          wrap = el.find(".techalgospotlight-control-wrap");

        el.addClass("techalgospotlight-control-responsive");
          ...
          for (var device in self.options.responsive) {
            $switcher.append(
              '<li class="' +
                ...
                device +
                '</span><i class="' +
                self.options.responsive[device].icon +
                '"></i></span></li>'
            );
          }
      ...
      _initUnit: function () {
        ...
        var template = wp.template("techalgospotlight-control-unit");
        ...
        if ("object" === typeof self.options.unit) {
          ...
          if ("undefined" !== typeof self.options.value.unit) {
            wrap
              .parent()
              .find(
                "#" + self.options.id + "-" + self.options.value.unit + "-unit"
              )
              .click();
          }
          ...
          $(
            '<span class="techalgospotlight-range-unit">' +
              self.options.unit +
              "</span>"
          ).insertAfter(wrap.find(".techalgospotlight-range-input"));
        }
    ...
    $.widget("techalgospotlight.rangeControl", RangeControl);
  })(jQuery);
})(jQuery);
