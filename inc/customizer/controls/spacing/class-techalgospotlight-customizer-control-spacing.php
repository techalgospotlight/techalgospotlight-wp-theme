<?php
/**
 * techalgospotlight Customizer custom spacing control class.
 *
...
if (!class_exists('techalgospotlight_Customizer_Control_Spacing')):
	/**
	 * techalgospotlight Customizer custom select control class.
	 */
	class techalgospotlight_Customizer_Control_Spacing extends techalgospotlight_Customizer_Control
	{
		 ...
		 * @var string
		 */
		public $type = 'techalgospotlight-spacing';
		...
		public function to_json()
		{
			...
			$this->json['title'] = esc_html__('Link values', 'techalgospotlight');
		...
		protected function content_template()
		{
			?>
			<div
				class="techalgospotlight-control-wrapper techalgospotlight-spacing-wrapper<# if ( data.responsive ) { #> techalgospotlight-control-responsive <# } #>">
						...
						<# if ( ! _.isEmpty( data.unit ) ) { #>
							<div class="techalgospotlight-control-unit">
								<# _.each( data.unit, function( unit ){ #>
									<input type="radio" id="spacing-unit-{{ data.id }}-{{ unit }}" name="spacing-unit-{{ data.id }}"
										value="{{ unit }}" <# if ( unit===data.value['unit'] ) { #> checked="checked"<# } #> />
							...
							<# } #>
										...
										<# _.each( data.responsive, function( settings, device ){ #>
											<ul class="{{ device }} control-responsive" data-device="{{ device }}">
												...
												<# _.each( data.choices, function( title, id ){ #>
													...
													<# if ( _.isEmpty( data.value[ device ] ) ) { data.value[ device ]=[]; } #>

														<li class="spacing-control-wrap spacing-input">
															<input {{{ data.inputAttrs }}} name="spacing-control-{{ device }}-{{ id }}" type="number"
																data-spacing-choice="{{ id }}" value="{{{ data.value[ device ][ id ] }}}"
																data-default="{{ data.default[ device ][ id ] }}" />
															...
															<# if ( _.size( data.choices )> 1 ) { #>
																<li class="spacing-control-wrap">
																	<div class="spacing-link-values">
																		<span class="dashicons dashicons-admin-links techalgospotlight-spacing-linked"
																			data-element="{{ data.id }}" title="{{ data.title }}"></span>
																		<span class="dashicons dashicons-editor-unlink techalgospotlight-spacing-unlinked"
																			data-element="{{ data.id }}" title="{{ data.title }}"></span>
																	</div>
																</li>
																<# } #>
														...
														<# _.each( data.choices, function( title, id ){ #>
															...
															<li class="spacing-control-wrap spacing-input">
																<input {{{ data.inputAttrs }}} name="spacing-control-{{ id }}" type="number"
																	data-spacing-choice="{{ id }}" value="{{{ data.value[ id ] }}}"
																	data-default="{{ data.default[ id ] }}" />
																<span class="techalgospotlight-spacing-label">{{{ title }}}</span>
															</li>
								...
								</div><!-- END .techalgospotlight-control-wrap -->
