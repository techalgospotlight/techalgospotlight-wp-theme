(function ($) {
  ...
  wp.customize.controlConstructor["techalgospotlight-spacing"] =
    wp.customize.Control.extend({
      ready: function () {
        ...
        // Change unit.
        control.container
          .find('.techalgospotlight-control-unit input[type="radio"]')
          .on("change", function () {
            control.save_value();
          });
      ...
      save_value: function () {
        var new_val = {},
          ...
          choice,
          device,
          units = this.container.find(".techalgospotlight-control-unit");

        if (devices === undefined || devices.length == 0) {
          for (choice in choices) {
            new_val[choice] = this.container
              .find('[data-spacing-choice="' + choice + '"]')
              .val();
          }
        } else {
          for (device in devices) {
            new_val[device] = {};
            for (choice in choices) {
              new_val[device][choice] = this.container
                .find(
                  ".control-responsive." +
                    device +
                    ' [data-spacing-choice="' +
