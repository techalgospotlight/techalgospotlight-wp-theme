    ...
    wp.customize.Control.extend({
      ready: function () {
        ...
        control.container.on(
          ...
          function () {
            control.update_font_weight_field(
              control.container.find("#font-family-" + control.params.id).val()
            );
        ...
        control.container.on(
          ...
          function () {
            control.update_font_subsets_field(
              control.container.find("#font-family-" + control.params.id).val()
            );
        ...
        control.container.find(".reset-defaults").on("click", function () {
          ...
          if ("font-family" in control.params.display) {
            control.populate_font_family_field();
            control.container
              .find("#font-family-" + control.params.id)
              .val(control.params.default["font-family"])
              .trigger("change");
          }
          ...
          selectFields.forEach((item) => {
            if (item in control.params.display) {
              control.container
                .find('[data-option="' + item + '"]')
                .val(control.params.default[item])
                .trigger("change");
            }
          });
        ...
        var outside_click_close = function (e) {
          if (
            ...
            !control.container.has($(e.target).closest(".popup-content"))
              .length &&
      ...
      update_value: function () {
        ...
        self.container.find("[data-option]").each(function () {
          ...
          if (
            "font-size-unit" === option ||
            "line-height-unit" === option ||
            "letter-spacing-unit" === option
          ) {
        ...
        if ("inherit" === font_family || "default" === font_family) {
          ...
          $.each(default_weights, function (index, variant) {
            ...
            options +=
              ...
              selected +
              ">" +
              self.params.l10n.weights[variant] +
              "</option>";
          });
        } else {
          $.each(
            techalgospotlight_typography_vars.fonts,
      ...
      update_font_subsets_field: function (font_family) {
        var self = this,
          ...
          selected = "",
          setting = self.setting.get(),
          field = self.container.find(
            ".techalgospotlight-typography-font-subsets"
          );
        ...
        $.each(
          ...
          function (group_id, group) {
            if ("undefined" !== typeof group.fonts[font_family]) {
              if (
                "system_fonts" === group_id ||
                "standard_fonts" === group_id
              ) {
                field.hide();
              } else {
                field.show();
              ...
              $.each(
                ...
                function (index, subsets) {
                  ...
                  if (
                    (setting["font-subsets"] &&
                      -1 !== setting["font-subsets"].indexOf(subsets)) ||
                    "latin" === subsets
                  ) {
                    selected = ' selected="selected"';
                  }
      ...
      populate_font_family_field: function (e) {
        ...
        selected =
          "inherit" === setting["font-family"] ? ' selected="selected"' : "";
        options +=
          '<option value="inherit"' +
          selected +
          ">" +
        ...
        selected =
          "default" === setting["font-family"] ? ' selected="selected"' : "";
        options +=
          '<option value="default"' +
          selected +
          ">" +
          self.params.l10n.default +
          "</option>";
